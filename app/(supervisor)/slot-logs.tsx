...
export default function SlotLogsScreen() {
  ...
  const formatDate = (dateStr: string) => {
    ...
    if (dateStr === today) return t('supervisor.slotLogs.today');
    if (dateStr === yesterday) return t('supervisor.slotLogs.yesterday');
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "bottom"]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('supervisor.slotLogs.title')}</Text>
      </View>
      ...
      >
        {loading ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>{t('supervisor.slotLogs.loading')}</Text>
          </View>
        ) : logs.length === 0 ? (
          <View style={styles.emptyContainer}>
            <ClipboardList size={48} color={COLORS.textMuted} />
            <Text style={styles.emptyText}>{t('supervisor.slotLogs.noLogs')}</Text>
            <Text style={styles.emptySubtext}>{t('supervisor.slotLogs.noLogsSubtitle')}</Text>
          </View>
        ) : (
                  ...
                  >
                    <View style={[styles.actionBadge, log.action === "started" ? styles.startBadge : styles.endBadge]}>
                      ...
                      <Text style={[styles.actionText, log.action === "started" ? styles.startText : styles.endText]}>
                        {log.action === "started" ? t('supervisor.slotLogs.started') : t('supervisor.slotLogs.ended')}
                      </Text>
                    </View>
